...
const TransactionTokenTransfer = () => {
    ...
    const fetchData = async (params) => {
        const { contractaddress, startBlock, endBlock, page, offset, sort } = params;
        const url = `${process.env.NEXT_PUBLIC_API_URL}/account/tokentx/${contractaddress}/${startBlock}/${endBlock}/${page}/${offset}/${sort}`;
    ...
    const columns = [
        ...
        columnHelper.accessor('value', {
            header: 'Amount',
            cell: info => {
                const tokenDecimal = info.row.original.tokenDecimal || '18';
                const value = info.getValue();
                const displayValue = (parseFloat(value) / Math.pow(10, parseInt(tokenDecimal))).toFixed(4);
                return (
                    <span className="font-semibold text-sm" title={`${value} (${tokenDecimal} decimals)`}>
                        {displayValue}
                    </span>
                );
            }
        ...
        columnHelper.accessor('tokenName', {
            ...
            cell: info => (
                <div className="flex flex-col">
                    ...
                    <span className="text-xs text-gray-500">{info.row.original.tokenSymbol}</span>
                </div>
            )
        }),
        columnHelper.accessor('contractAddress', {
            header: 'Contract',
            cell: info => (
                <a className="font-mono text-xs text-blue-600 hover:text-blue-800 cursor-pointer" href={`https://etherscan.io/address/${info.getValue()}`} target="_blank" title={info.getValue()}>
                    {info.getValue().slice(0, 6)}...{info.getValue().slice(-4)}
                </a>
            )
        }),
    ...
    return (
        <div className="max-w-7xl mx-auto p-6">
            <h2 className="text-2xl font-bold mb-6 text-gray-800">ERC-20 Token Transfer Viewer</h2>
            ...
            {hasSearched && !loading && tableData.length === 0 && !error && (
                <div className="bg-white shadow-lg rounded-lg overflow-hidden p-12">
                    <div className="text-center">
                        ...
                        <h3 className="mt-2 text-lg font-medium text-gray-900">No token transfers found</h3>
                        <p className="mt-1 text-sm text-gray-500">
                            No token transfers were found for contract <span className="font-mono">{formData.contractaddress}</span>
                        </p>
                        ...
                        <div className="flex flex-wrap justify-between items-center gap-4">
                            ...
                            {metadata && (
                                <div className="flex flex-wrap gap-4 text-sm text-gray-600">
                                    <span>
                                        <strong>Address:</strong> {metadata.contractaddress.slice(0, 10)}...{metadata.contractaddress.slice(-8)}
                                    </span>
                                    <span>
...
export default TransactionTokenTransfer;